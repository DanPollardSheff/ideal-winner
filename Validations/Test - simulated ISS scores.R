...
#Analysis###################
#set the data to be the baseline data
param_data_bc <- param_data
#Turn off parameter exporting, I don't need to record the parameters in this case
Param_export <- 0
##########################################################
...
population_ISS_over16_only <- "No" 
population_ISS_under16_only <- "Yes"
#Code to run one simulation run
det_ISS_u16 <- run_simulation(param_data_bc, 0,1,10000,"manual",0.8, 0.5,0)
#ISS over 16 only
population_ISS_over16_only <- "Yes" 
population_ISS_under16_only <- "No"
#Code to run one simulation run
det_ISS_o16 <- run_simulation(param_data_bc, 0,1,10000,"manual",0.8, 0.5,0)
#Whole population
population_ISS_over16_only <- "No" 
population_ISS_under16_only <- "No"
#Code to run one simulation run
det_ISS_wholepop <- run_simulation(param_data_bc, 0,1,10000,"manual",0.8, 0.5,0)

#produce tables of ISS score to check that the populations match my intended population
table(det_ISS_u16[,"ISS"])
table(det_ISS_o16[,"ISS"])
table(det_ISS_wholepop[,"ISS"])