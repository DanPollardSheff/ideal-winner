...
PSA_rand_no <-  4578 #random number to determine PSA parameters. #if -99 this will not change the seed after randomly determining the number of patients to run through the model. 
...
trauma_score_PSA <- run_simulation(param_data_bc, 1, PSA_numb, pat_numb, "manual",  0.391, 0.941,1)
write.csv(trauma_score_PSA, paste(file_location,"PSA results\\May21\\CRAMS_ISSu16",date,".csv", sep=""))
